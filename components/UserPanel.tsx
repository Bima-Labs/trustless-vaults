...
const DURATION_LABELS: { [key: number]: string } = {
  ...
  730: "2 Years",
  1095: "3 Years",
  [FIVE_MINUTES_IN_DAYS]: "5 Minutes",
};
...
const UserPanel: React.FC<UserPanelProps> = ({
  ...
  const [txProgress, setTxProgress] = useState<TxProgress>({
    status: "idle",
    txId: null,
    message: "",
    asset: null,
  ...
  const fetchUserTransactions = useCallback(async () => {
    ...
    console.log("BTC Address:", btcWalletState.address);
        ...
        (tx) =>
          (btcWalletState.address &&
            tx.userAddress?.toLowerCase() ===
              btcWalletState.address.toLowerCase()) ||
          (evmAddress &&
            tx.userEvmAddress?.toLowerCase() === evmAddress.toLowerCase())
      );
  ...
  // Fetch transactions when wallet addresses change
  useEffect(() => {
        ...
        if (btcWalletState.connected && window.unisat) {
          ...
          try {
            const response = await fetch(
              `${MEMPOOL_API_URL}/address/${btcWalletState.address}/utxo`
            );
            if (!response.ok) {
            ...
            console.error(
              "Failed to fetch tBTC balance from Mempool API:",
              err
            );
  ...
  useEffect(() => {
    ...
    if (isConfirmed) {
      setTxProgress({
        status: "confirmed",
        txId: evmTxHash ?? null,
        message: "Transaction confirmed! Stake is active.",
        asset: "wBTC",
      });
      // The receipt is confirmed, but we still need to ensure our backend is updated.
      // The pollForConfirmation function handles this.
      if (evmTxHash) {
        pollForConfirmation(evmTxHash, "wBTC");
      }
  ...
  const pollForConfirmation = useCallback(
    async (txId: string, asset: "tBTC" | "wBTC") => {
      setTxProgress({
        status: "pending",
        txId,
        message: "Waiting for blockchain confirmation...",
  ...
  const handleStake = async () => {
    ...
    try {
      ...
      if (selectedAsset === "tBTC") {
        ...
        txId = await window.unisat.sendBitcoin(vaultAddresses.btc, satoshis, {
          feeRate: FEE_RATE,
        });
      } else {
        // wBTC
        ...
        const chainId = chains.find(
          (c) => c.id === parseInt(EVM_NETWORK_ID, 10)
        )?.id;
        if (!chainId) {
        ...
        setTxProgress({
          status: "pending",
          txId: null,
          message: "Approving wBTC transfer...",
          asset: selectedAsset,
        });
        ...
        setTxProgress({
          status: "pending",
          txId: null,
          message: "Staking wBTC...",
          asset: selectedAsset,
        });

        let option;
        // Option 0 for long-term stake, Option 1 for short-term (5-min) buyback
        if (lockDuration === FIVE_MINUTES_IN_DAYS) {
          option = 1;
        } else {
          option = 0;
        }
      ...
      // Start polling for on-chain confirmation for both assets.
  ...
  const getExplorerUrl = (
    tx: Transaction | { txId: string; asset: "tBTC" | "wBTC" }
  ) => {
    if (!tx.txId) return "#";
        ...
        <div className="card">
          ...
          <div className="mb-4">
            ...
            <div className="flex bg-gray-200 dark:bg-gray-900/50 p-1 rounded-lg">
              <button
                onClick={() => setSelectedAsset("tBTC")}
                className={`w-1/2 py-2 text-sm rounded-md transition-colors ${
                  selectedAsset === "tBTC"
                    ? "bg-orange-500 text-white"
                    : "text-gray-500 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700"
                }`}
              >
                tBTC
              </button>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono">
                {!btcWalletState.connected
                  ...
                  : loadingBalances
                  ? "..."
                  : `${parseFloat(btcBalance).toFixed(6)} tBTC`}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Your wBTC Balance:</span>
              <span className="font-mono">
                {isMounted && isLoadingWbtcBalance
                  ? "..."
                  : `${
                      wbtcBalanceData?.formatted.substring(0, 8) ?? "0.00"
                    } wBTC`}
              </span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={
              !isWalletConnected ||
              txProgress.status === "pending" ||
              txProgress.status === "confirmed"
            }
          ...
          {txProgress.status !== "idle" && (
            <div className="mt-6 p-4 bg-gray-100 dark:bg-black/20 rounded-lg border border-gray-300 dark:border-white/10">
              <div className="flex items-center">
                {txProgress.status === "pending" && (
                  <SpinnerIcon className="animate-spin h-5 w-5 mr-3 text-orange-500" />
                )}
                {txProgress.status === "confirmed" && (
                  <CheckCircleIcon className="h-5 w-5 mr-3 text-green-400" />
                )}
                {txProgress.status === "failed" && (
                  <XCircleIcon className="h-5 w-5 mr-3 text-red-400" />
                )}
              ...
              {txProgress.txId && txProgress.asset && (
                <a
                  href={getExplorerUrl({
                    txId: txProgress.txId,
                    asset: txProgress.asset,
                  })}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-orange-500 text-xs mt-2 hover:underline flex items-center"
                >
                ...
                {loadingTxs ? (
                  ...
                  userTransactions.map((tx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {DURATION_LABELS[tx.lockDurationDays] ||
                          `${tx.lockDurationDays} days`}
                      </td>
                      <td className="px-6 py-4">
                        {tx.claimed ? (
